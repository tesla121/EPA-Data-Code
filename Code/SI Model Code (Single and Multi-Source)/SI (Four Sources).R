...
random=as.numeric(c(sample[1],sample[2],sample[3],sample[4])) #Randomly pick four sources
print(random)
t=0
neighbors=list()
temp_active=list()
	...
	if(length(unlist(perm_active))>=length(V(graph))*0.10) #Set infection size
			{
				...
				Jazz_Hetero_10_4S[[length(Jazz_Hetero_10_4S)+1]]=unlist(perm_active)
				print(unlist(length(perm_active)))	
			#}	
			#save(Jazz_Hetero_10_4S, file="./EPA-Data-Code/Infection Graphs (Multiple Source)/4/Jazz_Hetero_10_4S.RData")
